...
function setupCharacterAnimations(model, gltf) {
  ...
  const clips = gltf?.animations?.length ? gltf.animations : model.animations;
  if (!clips?.length) {
    console.warn('[Anim] У модели нет анимаций — T-поза');
    return;
  }
  ...
  const runClip =
    findAnimationClip(clips, ['run', 'running', 'jog', 'sprint']) ?? clips[0];
  const jumpClip = findAnimationClip(clips, ['jump', 'leap']);
  ...
  console.log(`[Anim] Запущен клип: "${runClip.name}"`);
}
...
function updatePlayer(delta) {
  ...
  if (state.isJumping) {
    ...
    if (playerRoot.position.y <= state.groundY) {
      ...
      if (jumpAction && runAction && playerMixer) {
